...
install_deps() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Termux + Python 3.13 下部分国内镜像尚未同步 setuptools，直接走官方源
        log_info "正在升级 pip / setuptools / wheel..."
        if ! PIP_INDEX_URL="https://pypi.org/simple" "$pip_python" -m pip install --upgrade pip setuptools wheel 2>&1; then
            log_warn "从 pypi.org 安装失败，尝试镜像回退..."
