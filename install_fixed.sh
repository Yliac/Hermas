...
check_optional_system_packages() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        # psutil 不支持 Android 平台源码编译，必须通过 Termux 预编译包安装
        if ! "$PYTHON_PATH" -c 'import psutil' >/dev/null 2>&1; then
            log_info "正在通过 pkg 安装 python-psutil（Android 不支持从源码编译 psutil）..."
            if pkg install -y python-psutil >/dev/null 2>&1; then
                log_success "python-psutil 已通过 pkg 安装"
            else
                log_warn "python-psutil 安装失败，后续 pip install 可能会因 psutil 编译失败"
            fi
        fi
    fi
}
...
install_deps() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        # 将系统级 python-psutil 复制到 venv，避免 pip 尝试从源码编译 psutil
        if [ "$USE_VENV" = true ]; then
            local sys_psutil_site venv_site
            ...
            if [ -n "$sys_psutil_site" ] && [ -n "$venv_site" ] && [ ! -d "$venv_site/psutil" ]; then
                log_info "正在将系统 psutil 复制到虚拟环境..."
                cp -a "$sys_psutil_site"/psutil* "$venv_site/" 2>/dev/null || \
                    ln -sf "$sys_psutil_site"/psutil* "$venv_site/" 2>/dev/null || true
            fi
        fi

        # 预装构建依赖（--no-build-isolation 模式下 pip 不会自动安装）
        log_info "正在预装构建依赖 maturin..."
        PIP_INDEX_URL="https://pypi.org/simple" "$pip_python" -m pip install maturin >/dev/null 2>&1 || true

        # --no-build-isolation：复用已装好的 setuptools/maturin，避免构建隔离环境下
        # 重新从镜像拉取失败（Tsinghua 镜像对 Python 3.13 同步不完整）
        if ! PIP_INDEX_URL="https://pypi.org/simple" pip_install_with_fallback "$pip_python" install --no-build-isolation -e '.[termux]' -c constraints-termux.txt; then
            log_warn "Termux 扩展安装（.[termux]）失败，正在回退到基础安装..."
            if ! PIP_INDEX_URL="https://pypi.org/simple" pip_install_with_fallback "$pip_python" install --no-build-isolation -e '.' -c constraints-termux.txt; then
